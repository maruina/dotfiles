...
export interface WorkspaceOverride { rootMarker: string; command?: string; args?: string[]; env?: Record<string, string>; initializationOptions?: unknown; root?(filePath: string, workspaceRoot: string): string }
export interface ServerConfig { id: string; label: string; command: string; args: string[]; extensions: string[]; rootMarkers: string[]; initializationOptions?: unknown; settings?: unknown; env?: Record<string, string>; workspaceOverrides?: WorkspaceOverride[]; matches?(filePath: string): boolean; languageId(filePath: string): string; packageContext?(filePath: string, workspaceRoot: string, symbols: DocumentSymbol[], diagnostics: Map<string, Diagnostic[]>): unknown }
