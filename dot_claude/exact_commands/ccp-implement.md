...
model: sonnet
allowed-tools: Read, Write, Edit, Bash(git:*), Bash(bzl:*), Bash(codex:*), Bash(make:*), Bash(mkdir:*), Grep, Glob, Agent, AskUserQuestion, Skill
---
...
2. **Read anchor files** to establish implementation patterns. Do NOT skip any:
...
5. **Quick research** — Agent tool, `subagent_type: "Explore"`, thoroughness `"medium"`:
...
9. **Confirm** via `AskUserQuestion`: "Ready to implement: <plan title>. Proceed?"

## Phase 3: Implement

10. **Implement changes**, following the Approach and Key Design sections:
...
11. **Run Gazelle** for each touched component (before building):
...
12. **CRD codegen** — run only if `types.go` or `conditions.go` was modified:
    ```bash
...
13. **Run tests** for each touched component:
...
15. **Invoke the `simplify` skill.** It reads recently modified files, fixes redundancy, unnecessary abstractions, and inconsistent patterns, then re-stages its changes.

16. **Re-run tests** for all touched components (step 13) after simplification. All must pass before proceeding.

## Phase 5: Parallel Review

Spawn both reviewers simultaneously. Both report findings — neither edits files.

17. **Claude code reviewer** — Agent tool, `subagent_type: "feature-dev:code-reviewer"`, `model: "sonnet"`:
...
18. **Codex reviewer** — Agent tool, `subagent_type: "general-purpose"`, `model: "haiku"`:
...
20. **Write `$ARTIFACT_DIR/IMPLEMENTATION-REVIEW.md`** (local only, not committed):
...
21. **Present the review.** Summarize by count (e.g., "1 CRITICAL, 2 MAJOR, 4 MINOR").

22. **Ask** via `AskUserQuestion`: "Which findings should I fix?" Options: **All** / **CRITICAL only** / **Let me choose**.

## Phase 7: Fix

23. **Fix accepted findings.** Explain any deviation from the plan.

24. **Re-run tests** for all touched components (step 13). All must pass.

## Phase 8: Commit & PR

25. **Format check** (again after fixes):
    ```bash
    git diff --name-only origin/main...HEAD | grep '\.go$' > /tmp/pr_changed.txt
    bzl run @//tools/format:format_go -- --git-diff-file=/tmp/pr_changed.txt
    ```

26. **Commit changes** using atomic commits — one per logical topic (e.g., CRD types, controller, tests, worker). From `computecla-controller/CLAUDE.md`:
    ```bash
...
27. **Push and create PR** using the format from `computecla-controller/CLAUDE.md`:
    ```bash
...
28. **Trigger Codex review:**
    ```bash
    gh pr comment <pr-number> --body "@codex review"
    ```

29. **Present the PR link.** Do NOT merge.
