---
description: Create a PR with structured description (What/Why/Reviewer guide/Tests) and per-topic commits. Usage: /pr-create [base-branch]
argument-hint: <optional base branch, defaults to main>
allowed-tools: Read, Glob, Grep, Bash(git:*), Bash(gh:*), Skill, AskUserQuestion
---
...
Invoke the `rewrite` skill now. Use its principles as a style guide for everything you write in this command. Do not output anything yet.
...
gh pr view --json title,body,number 2>/dev/null || echo "no PR yet"
```

Also read any `CLAUDE.md` in the repo root and in the package(s) being changed — they may define required PR structure or context.
...
- Identify the logical topics — a topic is a self-contained concern that a reviewer can evaluate independently (e.g. "CRD API field", "webhook logic", "reconciler update")
...
**If tangled** (more than ~5 commits and they mix topics), ask the user:
...
1. `git branch backup/<branch>-pre-rebase` (safety net)
...
5. Verify: `git status` clean, `git diff backup/<branch>-pre-rebase..HEAD` shows only intentional additions
...
If the branch has no upstream or is behind:
...
Two to four sentences. The problem this solves. Why now. Links to tickets, incidents, or issues. Write as if returning to this code in 9 months with no other context — the reader must understand why this change exists, not just what it does.
...
Bullet list. What tests were added, what scenarios they cover, and — if no tests — why they weren't needed or where coverage already exists.
```
...
- Commit: 8-char SHA as clickable link to GitHub commit
...
Title format: `[TICKET] type(scope): subject` where type is `feat`, `fix`, `refactor`, or `docs`.

## Phase 8: Report

Print:
- PR URL
- The 6-row reviewer guide (topics, commit links, files) as a compact summary
