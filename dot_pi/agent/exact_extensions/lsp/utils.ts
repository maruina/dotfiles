...
export function findAncestorWithMarker(start: string, marker: string): string | null { const resolved = resolve(start); let dir = existsSync(resolved) && statSync(resolved).isFile() ? dirname(resolved) : !existsSync(resolved) && extname(resolved) ? dirname(resolved) : resolved; while (true) { if (existsSync(join(dir, marker))) return dir; const parent = dirname(dir); if (parent === dir) return null; dir = parent; } }
