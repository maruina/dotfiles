 ...
 *
 * ctrl+shift+space — toggle: idle → start recording, recording → stop + transcribe + auto-submit.
 ...
 * (escape, ctrl+*, CSI \x1b…) so the registered `ctrl+shift+space` shortcut and our raw-input
...
class DictationDisplay extends CustomEditor {
	...
	override handleInput(data: string): void {
		...
		// app keybindings, the ctrl+shift+space shortcut, and the onTerminalInput esc-to-abort
	...
	private indicatorLine(): string {
		if (this.phase === "recording") {
			...
			const hints = this.appTheme.fg("muted", "ctrl+shift+space stop · esc abort");
...
export default function dictateExtension(pi: ExtensionAPI): void {
	...
	const toggle = async (ctx: ExtensionContext): Promise<void> => {
			...
			if (session) {
				...
				if (!transcript || isWhisperSilenceHallucination(transcript)) {
					...
					// accidentally toggling ctrl+shift+space twice with nothing in between is common enough
			...
			// one. A non-empty editor means the user typed ctrl+shift+space when they meant to type
			...
			const unsubscribeEsc = ctx.ui.onTerminalInput((data) => {
				...
				// Bare spacebar also stops + transcribes — same as ctrl+shift+space. The custom
	...
	pi.registerShortcut("ctrl+shift+space", {
		description: "Toggle dictation (sox + MacWhisper). Space or ctrl+shift+space stops; esc aborts.",
		handler: toggle,
	});
}
