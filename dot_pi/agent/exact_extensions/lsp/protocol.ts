...
export interface ServerConfig { id: string; label: string; command: string; args: string[]; extensions: string[]; rootMarkers: string[]; initializationOptions?: unknown; languageId(filePath: string): string; packageContext?(filePath: string, workspaceRoot: string, symbols: DocumentSymbol[], diagnostics: Map<string, Diagnostic[]>): unknown }
