...
### 3. Route and examine
Identify explicit URL, error-family, service, cluster, namespace, environment, or system triggers. For every route, load a skill only when its description explicitly matches a supplied identifier or confirmed evidence, and load that matching skill before choosing domain commands. Do not infer a route from a plausible correlation. If no skill description explicitly matches, emit a bounded source-of-truth gap and ask for a pointer. Start with the smallest high-signal, read-only query and bound time range, result count, history depth, and fan-out. Treat an ambiguous environment, account, cluster, namespace, domain, or context as a stop condition when it could alter the answer.

An Atlas or Temporal workflow URL (host `atlas.ddbuild.io` or `temporal.ddbuild.io`) explicitly routes to `atlas-workflows`. Use that skill to collect a bounded visible failure chain. That skill determines the correct Atlas context/domain and gathers status, failure tree, workflow metadata, input, output, or bounded history. If collected evidence establishes a Datadog telemetry investigation, route to `datadog-mcp`, which determines the organization and constrains query shape.
